...
**Protocol scope: v0.1 · Document revision: 0.5 (2026-06-26).**
...
generated by `reference/are_generate.py`, the MAINNET real-data vector by `reference/are_real_vectors.py`, and all
are re-checked by `reference/run_vectors.py` (7/7 passing):
...
4. `reject_quorum_too_low` (step 3), `reject_bad_bls` (step 4), `reject_unproven_absence` (step 8),
   `reject_mixed_root_batch` (step 9) — one MINIMAL vector per failure class.

**Caveats — v0.5 status (honest, not hidden).** As of v0.5 the three v0.4 simplifications are **discharged by the
...
**Remaining before `draft`:**
- **Benchmarks** (which MUST state CPU/RAM, native-vs-WASM, clustered-vs-scattered) are still TODO. *(Indicative:
  the real 512-pubkey `FastAggregateVerify` runs in ≈4 s on an 8 GB laptop for a one-time vector build; not yet a
  rigorous benchmark.)*
- **Accept coverage for the deep `historical_summaries` ancestor path (step 7) and `provider_sig` (step 10)** — the
  real vector is a trivial-ancestor FINALIZED case (`read_block_header == finalized_header`); both paths exist in
  the verifier but are not yet locked by an accepting vector.
