...
- **Signing domain:** `compute_domain(DOMAIN_SYNC_COMMITTEE = 0x07000000, fork_version, genesis_validators_root)`.
  ...
  configuration**, never from envelope bytes (see §Security, domain pinning).
...
ProofFormat {
    trie_type: uint8    # 0 = hexary-MPT (current), 1 = binary-7864 (future)
    hash_fn:   uint8    # 0 = keccak256, 1 = blake3, 2 = poseidon2
    encoding:  uint8    # 0 = RLP, 1 = SSZ
}

ConsensusAnchor {            # referenced by anchor_ref; NOT inlined per envelope on the wire
    attested_header:    BeaconBlockHeader      # the sync-committee-signed header
    sync_committee_bits: Bitvector[512]
    sync_committee_signature: Bytes96          # BLS aggregate over the attested_header signing root
    execution_payload_header_branch: List[Bytes32]  # Merkle branch: state_root -> attested_header.body_root
    finality_branch:    List[Bytes32]          # present iff finality_status == FINALIZED
    finalized_header:   BeaconBlockHeader       # present iff finality_status == FINALIZED
    ancestor_proof:     List[Bytes32]          # proves block_number's header is an ancestor of finalized_header
}
```

Constants for v0.1: `MAX_READS = 256`, `MAX_NODES = 64`.
...
2. Obtain, from a beacon node, the `ConsensusAnchor` for the beacon block whose execution payload carries
   `state_root`: the sync-committee-signed `attested_header`, the `sync_committee_bits` + aggregate signature,
   and the `execution_payload_header_branch` from `state_root` up to `attested_header.body_root`.
3. If a FINALIZED envelope is requested, additionally include the `finality_branch`, `finalized_header`, and an
   `ancestor_proof` from `block_number`'s header to `finalized_header`.
4. Set `proof_format`, `finality_status`, `anchor_type`, `chain_id`, `timestamp`, `beacon_slot`.
...
5. **State-root binding (cross-field).** Verify the execution `state_root` is the one committed by the signed
   header — do NOT trust `envelope.state_root` as a bare field:
   ```
   derived_state_root := verify_merkle_branch(
       leaf  = envelope.state_root,
       branch = anchor.execution_payload_header_branch,
       root  = anchor.attested_header.body_root )
   assert(derived_state_root == true)
   assert(beacon_slot_maps_to(envelope.block_number, anchor.attested_header))   # exec payload block ↔ beacon slot
   ```
...
7. **Finality entailment (if claimed).** If `envelope.finality_status == FINALIZED`:
   ```
   assert(verify_merkle_branch(anchor.finalized_header, anchor.finality_branch, anchor.attested_header.state_root))
   assert(verify_ancestor(envelope.block_number_header, anchor.ancestor_proof, anchor.finalized_header))
   ```
   An envelope labelled FINALIZED without a verifiable `finality_branch` + `ancestor_proof` MUST be rejected.
   An `OPTIMISTIC_HEAD` envelope MUST NOT be accepted as dispute/audit evidence (see §Verifier obligations).

8. **Per-read Merkle verification (inclusion AND exclusion).** For each `ReadProof r` in `envelope.reads`:
   ```
   if r.presence == 0:   # inclusion
       assert(verify_account_proof(r.address, r.account_proof, root = derived_state_root) yields account A)
       if r.read_kind == storage:
           assert(verify_storage_proof(r.slot, r.storage_proof, root = A.storageRoot) yields r.value)
       else:
           assert(field_of(A, r.read_kind) == decode(r.value))
   else:                 # exclusion / proof-of-absence
       assert(verify_exclusion(r.address_or_slot, r.account_proof_or_storage_proof, root) == true)
   ```
   A bare "zero / not found" value with no exclusion proof MUST be rejected. The verifier MUST NOT default-accept
   an unproven absence (truncated MPT paths are a forgery vector).

9. **Batch root consistency.** `assert(all reads in envelope.reads verified against the same derived_state_root)`.
...
`signing_root`, `derived_state_root`, and per-read results). This is the single most important gap to close
...
1. chain_id/anchor_type/version match → pass.
2. `hash_tree_root(ConsensusAnchor) == anchor_ref` → pass.
...
5. `verify_merkle_branch(state_root=0x1111…, execution_payload_header_branch, attested_header.body_root)` → pass;
   block 21_000_000 ↔ beacon_slot 8_999_990 → pass.
6. `8_999_990 ≥ 9_000_000 − 64`? → **9_000_000 − 64 = 8_999_936; 8_999_990 ≥ 8_999_936 → pass.** Monotonic floor
   updated.
7. finality_status = OPTIMISTIC → skip; envelope is NOT admissible as dispute evidence.
8. `verify_account_proof(0xabcd…, account_proof, root=state_root)` → account A; `A.balance == 0x0de0b6b3a7640000`
   → pass.
